...
- **LF:fake-confidence-floor** — `max(0.3, ...)` on archive relevance showed a flat "güven %30" for relevant and irrelevant matches alike (#43). Never floor confidence to a constant; confidence must reflect actual relevance. Fix sites: `chat/conversation.py` (#43), `graph/pipeline.py` + `v2/engine/engine.py` (fixed later, after the first fix missed them — grep for the pattern when fixing an LF).
